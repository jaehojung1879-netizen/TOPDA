...
CI로는 못 채운다. 2026-08-01 실측으로 **50분 예산에 24개**였다(단지당 74초 — 호출이 느리고
37%가 HTTP 500·타임아웃으로 실패한다). 그 속도면 16,000개에 2년이라 사실상 안 채워진다.
그래서 전량은 로컬에서 한 번에 끝낸다:

```bash
cd _meta
DATA_GO_APT_PRICE=발급받은키 python collect_official_price.py --full
```
...
| | |
|---|---|
| 시간 예산 | 끈다 (`TIME_BUDGET_MIN` 무시). 끝까지 돈다 |
| 동시 호출 | 8개 단지 동시 (`--workers N`으로 조절) |
| 재시도·타임아웃 | 1회·15초로 줄인다 — 죽은 지번에 1분씩 쓰지 않도록 |
| 진행 표시 | 100개마다 성공/없음/실패와 **남은 시간** |
| 체크포인트 | 120초마다 저장 |

**언제 끊겨도 이어받는다.** Ctrl-C·일일 한도 초과·정전 어느 쪽이든 저장하고 끝내며,
같은 명령을 다시 돌리면 채운 단지는 건너뛴다. 시험 삼아 돌려 보려면 `--limit 50`.

#### 일일 요청 한도

data.go.kr은 한도를 넘겨도 **HTTP 200에 `<item>` 없는 봉투**(`returnReasonCode` 22)를 준다.
이걸 '데이터 없음'으로 세면 남은 만 몇 천 개를 전부 헛치고 '응답없음'으로 기록해 버린다.
`lib_pdata`가 이 봉투를 `TrafficExceeded`로 올리고, 수집기는 즉시 저장하고 멈춘다 —
로그에 "일일 요청 한도 초과"가 찍히면 **내일 같은 명령을 다시 돌리면 된다.**

단지 하나에 호출 6회이므로 16,000개면 약 10만 호출이다. data.go.kr 개발계정 기본 한도
(일 10,000건)로는 며칠에 나눠 돌게 되고, 운영계정으로 상향하면 한 번에 끝난다.

#### 응답 없음·실패 기록

`data/official_price_misses.json`에 '지역키|단지명 → 마지막 실패일'을 적어 45일간 다시
부르지 않는다. 대장에 주택가격이 없는 지번(오피스텔·단독 등)과 서버가 계속 500을 뱉는
지번이 섞여 있는데, 둘 다 매 실행 다시 부르면 시간의 상당 부분을 거기에 쓴다.
CI 워크플로도 이 파일을 함께 커밋한다.

#### 끝낸 뒤

```bash
cd ..
git add site/assets/official_price.json data/official_price_misses.json
git commit -m "공동주택 공시가격 전량 적재"
git push
```

전량 파일은 약 2 MB다(단지당 144 bytes). 채우고 나면 CI의 매일 실행은 대상이 0이라 사실상
아무 일도 하지 않는다 — 공시가격은 연 1회 갱신이므로 그게 정상 상태다.
...
- **`collect_bond_rate.py`는 공식 오픈API가 아니라 페이지 파싱**입니다(국민주택채권 할인율은 공공데이터포털에 깨끗한 API가 없음). 실패해도 기존 값을 유지하므로 사이트는 안 깨지지만 값이 멈춥니다 — Actions 로그의 `[bond_rate]` 라인으로 확인하세요.

  **2026-08-04에 몇 주간의 수집 정지를 해결했습니다.** 원인은 파서가 아니었습니다 —
  KB `quics?page=C028010` 은 데이터 페이지가 아니라 **조회 폼**입니다. 조회를 누르기 전에는
  표가 존재하지 않으므로, GET 응답에는 메뉴 텍스트만 있었고 어떤 파서로도 값을 찾을 수
  없었습니다. "표 구조 파서가 열을 잘못 짚는다"고 본 것은 오진이었습니다.

  지금은 조회 버튼(`uf_doInquiry()`)이 하는 일을 그대로 POST 로 재현합니다 — 브라우저 없이
  표준 라이브러리만 씁니다. 응답 표에서 값을 고를 때 함정이 둘 있으니 손대기 전에 알아두세요:

  - 표가 **내림차순**입니다(최신이 맨 위). "마지막 행이 최신"으로 가정하면 그 달의 가장
    오래된 값을 집습니다.
  - **미래 날짜가 섞입니다.** 은행이 다음 영업일 값을 미리 올립니다. 최댓값을 고르면 오늘
    쓸 수 없는 값이 들어옵니다. 그래서 '오늘 이하 중 가장 최근' 행을 고릅니다.

  소스로 쓸 수 없는 곳(2026-08-04 실측): 우리은행(`svc.wooribank.com`)은 GitHub 러너에서
  정적·렌더 모두 60초 타임아웃, 주택도시기금 `FP070503.jsp` 는 렌더해도 본문이 882자뿐
  (제목·발자취·푸터만, 내용 영역이 비어 있음). 할인율은 은행마다 다른 값이 아니라 전국
  공통 기준값이므로 응답하는 곳 하나면 됩니다.

  ⚠ 개발 환경에서는 이 도메인들이 전부 막혀 있어(HTTP 000/403) 응답을 볼 수 없습니다.
  **진단·검증은 반드시 Actions 에서 하세요** — `probe-bond-rate.yml`(수동 실행)이 소스 응답을
  찍고, 마지막 스텝에서 수집기를 실제로 돌려 결과 JSON 을 그대로 보여줍니다. 이 워크플로는
  커밋 권한이 없어 저장소에 영향을 주지 않습니다.
