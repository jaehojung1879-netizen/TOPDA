...
- **`collect_bond_rate.py`는 공식 오픈API가 아니라 페이지 파싱**입니다(국민주택채권 할인율은 공공데이터포털에 깨끗한 API가 없음). 실패해도 기존 값을 유지하므로 사이트는 안 깨지지만 값이 멈춥니다 — Actions 로그의 `[bond_rate]` 라인으로 확인하세요.
  ...
  ⚠ **표 파서를 고치는 것으로는 해결되지 않습니다.** 2026-08-04 진단(`probe-bond-rate.yml`)에서
  확인한 사실:

  - KB `quics?page=C028010` 은 데이터 페이지가 아니라 **조회 폼**입니다. 브라우저로 렌더해도
    "기준년도/기준월 … 년 / … 월 [조회]" 만 있고, 조회를 누르기 전에는 표가 존재하지 않습니다.
    지금까지 "표 구조 파서가 열을 잘못 짚는다"고 본 것은 오진이었습니다 — 짚을 표가 없었습니다.
  - 같은 페이지 소스에 JSON 엔드포인트가 있습니다:
    `/quics?page=C028010&QAction=556954&RType=json&USER_TYPE=02` (556956 도 있음).
    폼 `IBS` 의 파라미터는 한글 이름입니다: `기준년월일` · `조회구분` · `요청페이지`.
  - 우리은행(`svc.wooribank.com`)은 GitHub 러너에서 정적·렌더 모두 60초 타임아웃입니다 —
    소스로 쓸 수 없습니다.
  - 주택도시기금 `FP070503.jsp` 는 정적 GET 이 메뉴 껍데기만 주고, 렌더는 60초 안에 끝나지
    않습니다.

  개발 환경에서는 이 도메인들이 전부 막혀 있어(HTTP 000/403) 응답을 볼 수 없습니다. 그래서
  진단은 반드시 Actions 에서 하세요 — `probe-bond-rate.yml` 을 돌리면 정적 요청과 브라우저
  렌더 양쪽 결과가 로그로 나옵니다. 이 워크플로는 값을 저장하거나 커밋하지 않습니다.
