...
# 생성 기준 (2026-07-26 샘플 23개 검토 후 확정)
#   3건 → 5건으로 올렸다. 4~5건짜리 페이지는 본문이 1,770자인데 그중 상당수가 면책·출처
#   같은 정형 문구여서, 단지 고유 정보는 표 몇 줄이 전부였다(40건 이상은 4,677자).
#   기준을 5건으로 올리면 대상이 4,971 → 4,553개로 418개만 줄면서 가장 얇은 구간이
#   빠지고, 추이 차트를 갖는 페이지 비율이 76% → 82%로 오른다.
#   세대수 100은 유지한다. 300으로 올리면 2,941개로 35%가 날아가는데, '단지명 실거래가'
#   롱테일 검색을 노리는 페이지라 커버리지를 그만큼 포기할 이유가 없다.
MIN_DEALS = 5
MIN_HOUSEHOLDS = 100
WINDOW_MONTHS = 12
