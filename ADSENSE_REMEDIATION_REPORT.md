...
**임의로 만들지 않은 항목** (6항 ‘운영자가 확인해야 하는 항목’ 콜아웃에 명시):
Supabase 데이터 리전·백업 보관 기간, GA4 데이터 보관 기간. 저장소 코드로 확인할 수 없어
구체적 국가명·기간을 적지 않았습니다.
...
→ 구글 Search Console 소유확인은 8절 ‘남은 수동 작업’ 참고.
...
1. **연락처 전자우편** — 저장소 어디에도 사이트 연락처로 선언된 이메일이 없습니다.
   `contact.html`의 “전자우편” 칸과 `privacy.html` 13항이 “운영자 확인 후 게시”로 비어
   있습니다. `CONTACT_EMAIL_PLACEHOLDER`를 화면에 노출하지 않았습니다.
   → git 커밋 메타데이터에 개인 이메일이 있으나, 사이트 연락처로 공개하는 것은 운영자
   결정 사항이라 임의로 게시하지 않았습니다.
2. **구글 Search Console 소유확인** — `index.html`의 플레이스홀더를 제거했으므로,
   DNS TXT 또는 HTML 파일 방식으로 확인해야 합니다(메타 태그 방식을 쓰려면 실제 토큰을
   주입하는 빌드 단계가 필요합니다).
3. **Supabase 데이터 리전·백업 보관 기간, GA4 데이터 보관 기간 확인** →
   `privacy.html` 6항 콜아웃과 9항, `en/privacy.html` 5·8항에 채워 넣기.
...
- [ ] 8절 ‘반드시 필요’ 3건을 처리했는지 (연락처 · Search Console 소유확인 · 보관 기간)
