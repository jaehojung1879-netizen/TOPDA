...
| 이전 서술 | 실제 |
|---|---|
| “별도 서버나 데이터베이스를 운영하지 않습니다” | 게시판·댓글이 Supabase에 저장됨 |
| “광고 식별자나 교차 사이트 추적은 사용하지 않습니다” | AdSense가 175개 페이지에서 가동 |
| “향후 분석 도구(예: GA4)를 도입할 경우” | GA4·네이버 애널리틱스가 전 페이지에서 가동 |
| 게시판 “같은 기기에 표시” | 공개글은 전 방문자에게 보임 |
| 제3자 제공 “현재 없음” | Google·네이버·Supabase·GitHub·jsDelivr로 전달 |

### 지금 담고 있는 것 (5개 절 · 2,068자)

1. **사용하는 도구와 처리 내용** — GA4(측정 ID·IP 익명화·담지 않는 값), 네이버 애널리틱스,
   AdSense(쿠키·광고 식별자·광고를 넣지 않는 페이지), Supabase(저장 항목·소유자 토큰),
   localStorage를 표 하나로 정리
2. **제3자 처리와 국외 이전** — 사업자별 역할과 전달 항목
3. **쿠키·광고 거부 방법** — 브라우저 설정, Google 광고 설정, GA 차단 부가기능
4. **보유·삭제와 이용자의 권리**
5. **문의** — 게시판(비밀글)

**임의로 만들지 않은 항목**: Supabase 데이터 리전·백업 보관 기간, GA4 데이터 보관 기간.
저장소 코드로 확인할 수 없어 구체적 국가명·기간을 적지 않고 “각 서비스의 정책과 프로젝트
설정에 따른다”고만 적었습니다. 공개 페이지에 “운영자가 확인해야 하는 항목” 같은 TODO 문구는
남기지 않습니다 — 사실 서술로 충분하고, TODO는 미완성 신호가 됩니다.

**연락 창구**: 게시판으로 문의·삭제 요청을 받습니다. 회원가입 없이 쓸 수 있고 비밀글이 되므로
개인정보 삭제 요청 창구로 기능합니다. 전자우편 주소는 두지 않으며, 운영자 개인 정보는
사이트 어디에도 표시하지 않습니다.
...
표본이 적은 단지 페이지에는 **그 사실을 방문자 관점에서** 알립니다
(“이 단지는 최근 12개월 유효 거래가 10건으로 표본이 적은 편입니다. … 가격대를 가늠하는
참고로 보시고, 서울 종로구의 다른 단지 시세도 함께 확인하시는 편이 좋습니다”).
