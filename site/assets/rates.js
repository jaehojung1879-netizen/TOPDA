  ...
  window.TOPDA_RATES = {
    ...
    dsr: {
      ...
      applicability: {
        ...
        //
        //  아래 목록은 **그 대출을 신규로 신청할 때** 차주단위 DSR 규제를 적용하지
        //  않는다는 뜻이다. 다른 대출(예: 주담대)을 신청하면 이 대출들도 **기존 부채로
        //  산입**된다 — 주택연금만 부채에서도 빠진다.
        //
        //  근거: 은행 여신업무기준 FAQ(2025-11) Q7 — "단, 위 항목에 해당하지 않는
        //  대출을 신규 취급하는 경우 위 항목의 대출은 DSR 산출을 위한 기존 부채
        //  금액에 포함됩니다.(주택연금(역모기지론)은 부채금액에서 제외)"
        //
        //  2026-08-02~08-04에 이 구분을 놓쳐 예적금담보대출을 부채에서도 0으로 뒀다.
        //  그 방향의 오차는 기존 부채를 줄여 **한도를 과대 산출**한다.
        exemptAsNewLoan: [
          '분양주택 중도금대출',
          '재건축·재개발 이주비대출, 추가분담금 중도금대출',
          '분양오피스텔 중도금대출',
          '서민금융상품(새희망홀씨·햇살론 등)',
          '300만원 이하 소액대출',
          '전세자금대출 (일반적으로 제외. 1주택자의 수도권·규제지역 신규대출은 이자만 반영)',
          '주택연금(역모기지론) — 유일하게 기존 부채에서도 제외',
          '정부·공공기관·지자체 협약(이차보전) 대출',
          '자연재해 지역 등 긴급 취급 대출',
          '보험계약대출(약관대출)',
          '상용차 금융',
          '예·적금 담보대출',
        ],
        exemptFromDebtToo: ['주택연금(역모기지론)'],
        // 옛 이름. 화면·테스트가 참조하던 키라 남겨 두되 뜻이 좁아졌음을 밝힌다.
        excludedFromDsr: [
          '전세자금대출 (일반적으로 제외. 1주택자의 수도권·규제지역 신규대출 등은 이자만 산입)',
          '중도금대출(집단대출)·재건축 이주비대출 — 잔금대출로 전환되면 그때부터 산입',
          '예·적금 담보대출, 보험계약대출(약관대출) — 단, 다른 대출 신청 시 기존 부채로는 산입',
          '서민금융상품, 정부·지자체 협약대출',
          '자연재해 지역 등 긴급대출, 300만원 이하 소액 신용대출',
        ],

        // ── DSR 연간 원리금 산정만기 ──
        //  은행 여신업무기준 참고사항 「DSR 산출시의 대출 연간 원리금 상환액 산정기준」
        //  표를 그대로 옮긴 것이다. 연간 원금상환액 = 대출총액 ÷ 아래 연수.
        assumedTermYears: {
          mortgageBullet: { years: 10, label: '주담대 원금일시상환', basis: '대출총액 ÷ 대출기간(최대 10년)', cap: true },
          progressPayment: { years: 25, label: '중도금대출·이주비대출', basis: '상환방식 무관' },
          creditAmortizing: { years: 5, label: '신용대출(특정 분할상환구조)', basis: '약정만기 5년 이상 10년 이내', min: 5, max: 10 },
          credit: { years: 5, label: '신용대출(그 외)', basis: '대출총액 ÷ 5년' },
          nonHouse: { years: 8, label: '오피스텔 外 비주택담보대출', basis: '상환방식 무관' },
          officetelBullet: { years: 8, label: '오피스텔 담보대출(원금일시)', basis: '상환방식별' },
          otherSecured: { years: 10, label: '기타담보대출', basis: '상환방식 무관' },
          cardLoanAmortizing: { years: 5, label: '장기카드대출(분할상환)', basis: '약정만기 5년 이내', cap: true },
          cardLoan: { years: 3, label: '장기카드대출(그 외)', basis: '약정만기 3년 이내', cap: true },
        },

        // ── 유가증권(주식)담보대출의 산정만기 = 8년 (2026-08-05 확정) ──
        //
        //  한때 두 갈래로 읽혔다. 기준표에 「오피스텔 外 비주택담보대출 8년」과
        //  「기타담보대출 10년」이 각각 있는데, 유가증권담보가 어느 쪽인지 각주만으로는
        //  갈리지 않았다. 실사용자 은행 DSR 원장 한 줄로도 판별이 안 됐다 —
        //  잔액 29,934,000 · 연간원리금 4,867,000 은 8년이면 금리 3.76%,
        //  10년이면 6.26% 로 **둘 다 성립**했기 때문이다.
        //
        //  차주가 그 대출의 실제 약정금리를 알려 주면서 갈렸다: **3.76%**.
        //
        //     8년:  29,934,000 ÷ 8  + 29,934,000 × 3.76% = 4,867,268원  ← 은행 4,867,000원
        //    10년:  29,934,000 ÷ 10 + 29,934,000 × 3.76% = 4,118,918원  ← 75만원 어긋남
        //
        //  8년 쪽이 268원(0.006%) 차이로 맞는다. 우연으로 보기 어려운 정확도다.
        //  (남는 전제 하나: 원장 조회일은 2026-04-14 이고 금리는 2026-08 기준으로
        //   들은 값이다. 그 사이 이 대출의 약정금리가 바뀌지 않았다고 본다.)
        securedLoanTerm: {
          years: 8,
          candidatesConsidered: [8, 10],
          resolvedAt: '2026-08-05',
          resolvedBy: '차주가 밝힌 실제 약정금리 3.76% — 은행 원장의 연간원리금 4,867,000원과 8년 가정에서만 일치',
          assumption: '원장 조회일(2026-04-14)과 금리 확인 시점(2026-08) 사이에 약정금리 변동 없음',
          confidence: 'verified',
        },
        includedWithAssumedTerm: [
          { kind: '유가증권(주식)담보대출', years: 8, confidence: 'verified' },
          { kind: '비주택담보대출', years: 8 },
          { kind: '신용대출', years: 5 },
          { kind: '마이너스통장(한도대출)', years: 5, basis: '한도금액 기준(실제 사용액 아님)' },
          { kind: '중도금·이주비대출(기존 부채일 때)', years: 25 },
        ],
        includedNote: '유가증권(주식)담보대출은 DSR 산정 제외 대상이 아닙니다. 산정만기 8년으로 「대출액÷8 + 이자」가 산입됩니다.',

        // 한도대출(마이너스통장)은 실제 사용액이 아니라 **한도금액** 기준이다.
        limitLoanBasis: '한도금액 (실제 사용액 아님)',
        note: '전 금융권 총 대출액이 1억원 이하이면 차주단위 DSR이 적용되지 않습니다. 이 계산기는 보수적으로 항상 DSR을 적용하므로, 요건에 해당하지 않으면 실제 한도는 더 높을 수 있습니다.',

        // ── 소득 산정: 장래소득 ──
        //  은행 내규로 「장래소득 증가가 예상되는 경우 그 증가분을 반영해 연소득을
        //  산정」할 수 있다(주담대 DSR·DTI 산출에 한함, 최근 2년 근로소득 입증자료 등
        //  요건 충족 시). 반영 폭과 요건이 은행마다 달라 계산기는 적용하지 않는다 —
        //  다만 "왜 은행 한도가 더 크게 나왔는지"의 흔한 이유라 화면에 밝힌다.
        //  실사례(2026-04): 원천징수 총급여 119,864,890원 → 은행 최종 연소득금액
        //  127,685,915원(약 6.5% 상향)으로 DSR이 산출됐다.
        futureIncome: {
          appliedInCalculator: false,
          scope: '주택담보대출 DSR·DTI 산출에 한정',
          note: '은행에 따라 「장래소득」(향후 소득 증가 예상분)을 반영해 연소득을 더 높게 잡을 수 있습니다. 최근 2년 근로소득 입증자료 등 요건과 반영 폭이 은행마다 달라 이 계산기는 반영하지 않습니다 — 실제 한도가 이 결과보다 높게 나올 수 있으니 은행에 확인하세요.',
          confidence: 'estimate',
    ...
    loan: {
      ...
      stress: {
        ...
        scope: {
          ...
          excluded: [
            '전세자금대출 — 일반적으로 DSR 산정 제외(1주택자의 수도권·규제지역 신규대출 등은 이자만 산입)',
